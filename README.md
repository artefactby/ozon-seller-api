...
- **Покрыт весь API, а не выборка методов.** Типы сгенерированы из официальной
  OpenAPI-спецификации (версия API 2.1): 458 операций, 2083 схемы. Любой путь
...
- **Встроенный ограничитель частоты запросов**, знающий лимиты Ozon — 50 запросов в
  секунду на Client-Id плюс пометодные лимиты из спецификации. Поставляется
  отдельным вложенным путём `/limiter`: не подключили — ничего за него не платите.
...
в лимиты вовсе, подключите встроенный ограничитель — он выстраивает вызовы заранее:
...
const client = new OzonClient({
  ...
  limiter: new TokenBucketLimiter({
    // Значения по умолчанию — ровно те лимиты, которые документирует Ozon:
    // 50 запросов в секунду на всё плюс пометодные ограничения из спецификации
    // (например, /v2/products/stocks — 80 в минуту). Сюда можно добавить
    // лимиты, измеренные вами самостоятельно:
...
Ограничитель работает в пределах одного процесса. Несколько процессов с одним
Client-Id получат каждый свой бюджет — для общего лимита реализуйте интерфейс
...
await client.request('/v1/actions', undefined, {
  ...
  priority: 10, // для ограничителя: выше — раньше
});
```
...
`multipart/form-data` (передайте `FormData`), двоичные тела и `ReadableStream` уходят
...
## Постраничная выгрузка — на вашей стороне

Пакет сознательно заканчивается на границе транспорта: один вызов — один логический
запрос к API. Постраничные циклы, сборка полных выборок, разбиение на партии — это
...
- Встроенный ограничитель не разделяет бюджет между процессами (см. выше).
